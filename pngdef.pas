...
type
  ...
  png_rw_ptr = procedure(png_ptr: Pointer;var data: Pointer; length: png_size_t);
               stdcall;
 ...
 *}
const PNG_INFO_gAMA: Cardinal = $0001; // ub: used Cardinal instead of Integer
...
function png_get_progressive_ptr(png_ptr: png_structp): png_voidp;
             stdcall;
procedure png_get_rgb_to_gray_status(png_ptr: png_structp);
             stdcall;
...
procedure png_set_read_user_transform_fn(png_ptr: png_structp;
             ...
             stdcall;
procedure png_set_rgb_to_gray(png_ptr: png_structp; error_action: int );
             stdcall;
...
function png_get_copyright(png_ptr: png_structp): png_charp; external pngDLL;
function png_get_header_version(png_ptr: png_structp): png_charp; external pngDLL;
procedure png_get_rgb_to_gray_status(png_ptr: png_structp); external pngDLL;
procedure png_set_gray_1_2_4_to_8(png_ptr: png_structp); external pngDLL;
procedure png_set_palette_to_rgb(png_ptr: png_structp); external pngDLL;
procedure png_set_rgb_to_gray(png_ptr: png_structp; error_action: int ); external pngDLL;
procedure png_set_tRNS_to_alpha(png_ptr: png_structp); external pngDLL;


procedure Check( const aProc );//ub added  
begin
  if @aProc = nil then
    raise Exception.Create( 'Error loading '+ pngDLL );
end;

initialization
  Check( png_read_info );
  // it'd be better check all procs otherwise error came only when you try to use it
